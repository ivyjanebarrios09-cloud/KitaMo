...
export default function YearlyStatementPage() {
  ...
  const filteredTransactions = transactions.filter(t => {
      ...
      return t.createdAt.toDate().getFullYear() === yearNumber;
  });
  ...
  const handleDownloadCSV = () => {
    if (loading || filteredTransactions.length === 0) {
        toast({ variant: 'destructive', title: 'No Data Available', description: 'There is no data to download for the selected year.' });
        return;
    }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold">Yearly Statement</h1>
            <p className="text-muted-foreground text-sm sm:text-base">Financial statement for {year} in {room?.name || '...'}</p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div ref={statementRef} className="bg-white rounded-lg shadow-md p-6 sm:p-10 text-black">
          <div className="space-y-8 max-w-4xl mx-auto">
            <div>
              <h1 className="text-2xl font-bold text-primary">Yearly Financial Statement</h1>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredTransactions.length > 0 ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="text-center h-24">No transactions for this year.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
            ...
            <div className="mt-8 space-y-2 text-right text-sm">
              <p><span className="text-muted-foreground">Total Income for {year}:</span> <span className='font-semibold'>PHP {totalIncome.toFixed(2)}</span></p>
              <p><span className="text-muted-foreground">Total Expenses for {year}:</span> <span className='font-semibold'>PHP {totalExpenses.toFixed(2)}</span></p>
            </div>
          </div>
        </div>
      )}
